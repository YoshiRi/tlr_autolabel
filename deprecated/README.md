...
- `export_awml.py` — built an AWML-style derived dataset (object_ann with db_tlr
  categories). **Replaced by `../to_object_ann.py`** (L2 A→B), which writes a
...
- `export_labels.py` — wrote COCO / CVAT-1.1 from our v1 sidecar. **Replaced by**
  producing standard `object_ann.json` (`../to_object_ann.py`) and letting the
...
and is applied by `../to_object_ann.py`.
